...
and an emulated DMA-block copy. A second opt-in layer reproduces the observed
factory GPIO startup levels and the GPIO1_IO26 100 ms low-to-high sequence.
The combined image is still marked **offline only** because the electrical
meaning and fail-safe polarity of GPIO1_IO26 and GPIO2_IO11 have not yet been
confirmed on the PCB.
...
Omit both feature options to produce the deliberately hardware-inert default.
The audio and board-control options are independent and both default off.
The post-link checker validates the complete vector table, including external
IRQ0 targeting the shared eDMA channel 0/16 handler. The source emulator
executes reset and initialization, checks the factory SAI1, PLL, pin, DMAMUX,
TCD, and optional GPIO values, verifies the delayed IO26 transition, injects
one RX block, invokes the source ISR, and verifies the unchanged TX block. It
does not access USB.
...
approved for streaming: digital passthrough and the observed GPIO sequence
are implemented, but the candidate GPIOs' electrical fail-safe polarity and
live slot semantics remain unverified.
...
2. Identify the converter/codec and prove the two remaining candidate GPIO
   meanings and fail-safe polarities on the PCB.
...
4. Add a safe target-visible heartbeat or measurement plan.
